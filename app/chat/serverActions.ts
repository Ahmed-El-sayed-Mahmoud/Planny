...
const messageServices = getInjection<IMessageServices>(DI_TYPES.IMessageServices);
...
export const LoadChatsAction =async ()=>{
    ...
    return await chatServices.getChatsByUserEmail();
}
export const LoadNewMessages=async (chatId:number)=>{
    return await messageServices.getMessagesByChatId(chatId);
}
export const SaveMessagesAction = async (msg: TablesInsert<'message'>, response: TablesInsert<'message'>) => {
